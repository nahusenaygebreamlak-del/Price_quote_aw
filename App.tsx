...
const App: React.FC = () => {
  ...
  const handleSavePDF = async () => {
    ...
    if (element) {
      try {
        // Create a hidden container for a high-quality capture
        ...
        // Clone the element into the container
        ...
        const canvas = await html2canvas(clone, {
          ...
          onclone: (clonedDoc) => {
            // Apply PDF-specific fixes to the cloned document
            const cells = clonedDoc.querySelectorAll('.pdf-cell, .pdf-value, .pdf-label');
            cells.forEach((el) => {
            ...
            // Force borders to be visible and clean
            const bordered = clonedDoc.querySelectorAll('.border, .border-b, .border-r');
            bordered.forEach((el) => {
              const htmlEl = el as HTMLElement;
              htmlEl.style.borderStyle = 'solid';
              htmlEl.style.borderColor = '#9ca3af'; // gray-400
            });
          }
        });
        ...
        pdf.save(`Quote_${state.quote.quoteNumber}.pdf`);
      } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 flex flex-col">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-200 px-6 py-4 flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 px-6 py-2.5 bg-[#004d73] hover:bg-[#003d5c] text-white rounded-lg shadow-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#004d73] disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {isGenerating ? (
          ...
          {isGenerating ? 'Export PDF' : 'Save as PDF'}
        </button>
      </header>
